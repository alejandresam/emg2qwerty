...
import torch
from torch import _functionalize_set_storage_changed, nn
...
class TDSLSTMEncoder(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        x, _ = self.lstm_layers(inputs)  # Apply LSTM layers
...
class RNNEncoder(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, rnn_type="LSTM", bidirectional=False):
        ...
        # Select LSTM or GRU based on config
        ...
        self.rnn = RNN(
            ...
            dropout=0.5 if num_layers > 1 else 0,  # Apply dropout only if num_layers > 1
            bidirectional=bidirectional,  # If True, forward + backward pass
            batch_first=False# Input shape (Batch, Time, Features)
        )
