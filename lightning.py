...
class TDSConvCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            ##nn.Linear(lstm_hidden_size * (2 if bidirectional else 1), charset().num_classes)
...
class RNNCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            RNNEncoder(
                ...
                rnn_type=rnn_type,  # Can be "RNN", "LSTM", or "GRU"
    ...
    def _step(self, phase: str, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # Update metrics
        metric_module = self.metrics[f"{phase}_metrics"]
        targets_np = targets.detach().cpu().numpy()
        target_lengths_np = target_lengths.detach().cpu().numpy()

        for i in range(len(targets_np[0])):
            target_i = LabelData.from_labels(targets_np[: target_lengths_np[i], i])
            metric_module.update(prediction=predictions[i], target=target_i)
